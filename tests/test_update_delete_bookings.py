def test_update_booking_successfully(logged_in_client):
    ...
    create_response = logged_in_client.post("/api/bookings", json=create_payload)
    ...
    update_response = logged_in_client.patch(f"/api/bookings/{booking_id}", json=update_payload)
...
def test_update_booking_frees_old_time_slot(logged_in_client):
...
def test_update_booking_rejects_conflicting_time_slot(logged_in_client):
...
def test_update_booking_rejects_missing_data(logged_in_client):
    ...
    response = logged_in_client.patch("/api/bookings/1", json=update_payload)
...
def test_update_booking_rejects_invalid_room(logged_in_client):
...
def test_update_booking_rejects_booking_not_found(logged_in_client):
    ...
    response = logged_in_client.patch("/api/bookings/999", json=update_payload)
    assert response.status_code == 404
...
def test_update_booking_rejects_other_users_booking(logged_in_client):
    ...
    response = logged_in_client.patch("/api/bookings/2", json=update_payload)
...
def test_delete_booking_marks_booking_as_cancelled(logged_in_client):
    # Tests that deleting a booking marks it as cancelled.
    # This is a soft delete, not a permanent delete.

    response = logged_in_client.delete("/api/bookings/1")
...
def test_deleted_booking_time_slot_becomes_available(logged_in_client):
    ...
    delete_response = logged_in_client.delete("/api/bookings/1")
...
def test_delete_booking_rejects_booking_not_found(logged_in_client):
    # Tests that deleting a booking fails if the booking ID does not exist.

    response = logged_in_client.delete("/api/bookings/999")
    assert response.status_code == 404
...
def test_delete_booking_rejects_other_users_booking(logged_in_client):
    # Tests that the current user cannot delete another user's booking.
    # Booking ID 2 belongs to user_id 2 in the mock data.

    response = logged_in_client.delete("/api/bookings/2")
    assert response.status_code == 403
